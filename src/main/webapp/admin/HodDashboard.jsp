...
<% String course = null;
	...
	String courseName = null;
    	...
    	try(Connection con = DbConnection.getConnection()){
    		String sql = "SELECT c.name as course, c.id, c.duration,c.full_name as courseName FROM course c, admin a WHERE c.hod_id = ?; ";
    ...
    <div><h1>Result Confirmation</h1></div>
      <div class="Result-Button1">
          <c:forEach var="b" items="${batches}">
			<button onclick="window.location.href ='../admin/ApproveResult?batch=${b.batchId}&courseId=<%=courseId%>'">${b.startingYear}-${b.endingYear}</button>
          </c:forEach>   
    </div>                      
  </div> 
</div>
...
<div id="ViewResult">
  <div id="Resultpopup-content">              
    ...
    <div><h1>View Results</h1></div>
      <div class="Result-Button1">
          <c:forEach var="b" items="${batches}">
          <button onclick="window.location.href ='../admin/getResult?batch=${b.batchId}&course=<%=courseName %>'">${b.startingYear}-${b.endingYear}</button>
          </c:forEach>   
    </div>                      
  </div> 
</div>
