...
<%@ page import="util.GetSemesterFromBatch" %>
<% String course = null;
   String CourseId=null;String courseName = null;
	HttpSession s = request.getSession(false);
	Student student = (Student)session.getAttribute("student");
	int id = student.getId();
	try(Connection con = DbConnection.getConnection()){
		String sql = "select course.name,course.id,course.full_name from batch join course on batch.course_id = course.id where batch.name =?";
		...
		if (rs.next()) {
			...
			courseName = rs.getString("full_name");
        } else {
...
boolean examForm = false;
try (Connection con = DbConnection.getConnection()) {
	    PreparedStatement ps = con.prepareStatement("SELECT * from exam_form where student_id = ? and semester = ?");
	...
	request.setAttribute("ef", examForm); %>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <div class="main-content">
       ...
       <div class="operations">
            <div class="operations-Button">
               ...
               <input type="hidden" name="course" value="<%=CourseId%>"><input type="hidden" name="cName" value="<%=courseName%>">
             ...
             <button onclick="window.open('https://www.antiragging.in/complaint_register_form.html', '_blank')">Anti Ragging</a></button>
            </div>
        </div>
    </div>
</main> 
...
<div id="Enroll">
    <div class="Enroll-popup">
		...
		<br>
          ...
          (*Note: You will be logged out after submission.)<p style="color:red">Please don't fill this form twice in a semester!</p>
...
<div id="examform">
    <div class="examform-popup">
        ...
        <hr color="black" height="2px">
		<c:choose><c:when test="${ef }">
			<center><h2>Already submitted</h2></center></c:when>
		<c:otherwise>
			<div><h2><form action="../student/examFormSubmit" method="post">
        <table class="table3">
                <tr>
                    <th>Name</th>
                    <td><%=student.getName() %></td>
